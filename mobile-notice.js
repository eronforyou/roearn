(function () {
  ...
  api.storage.local.get(['installTimestamp', 'mobileWarningShown'], async (result) => {
    const { installTimestamp, mobileWarningShown } = result;

    if (!installTimestamp) return;
    if (Date.now() - installTimestamp < FIVE_MINUTES_MS) return;
    if (mobileWarningShown === true) return;
  ...
  function dismiss() {
    const overlay = document.getElementById('roearn-mobile-warning');
    if (overlay) overlay.remove();
    api.storage.local.set({ mobileWarningShown: true });
  }

  function injectKeyframes() {
    if (document.getElementById('roearn-mobile-warning-css')) return;
    const style = document.createElement('style');
    style.id = 'roearn-mobile-warning-css';
    style.textContent = `
      @keyframes roearn-rainbow-flow {
  ...
  function showWarning() {
    if (document.getElementById('roearn-mobile-warning')) return;
    injectKeyframes();

    const overlay = document.createElement('div');
    overlay.id = 'roearn-mobile-warning';
    ...
    title.textContent = getMessage('mobileWarningTitle');
    title.style.cssText = `
      margin: 0;
      color: #e0e0e0;
      font-size: 17px;
    ...
    body.textContent = getMessage('mobileWarningBody');
    body.style.cssText = `
      margin: 0;
      color: #999;
      font-size: 14px;
    ...
    catalogBtn.href = 'https://www.roblox.com/catalog';
    catalogBtn.rel = 'noopener noreferrer';
    catalogBtn.textContent = getMessage('mobileWarningCatalogBtn');
    catalogBtn.style.cssText = `
      display: block;
      margin-top: 4px;
      background: linear-gradient(90deg, #6bb5ff, #a66bff, #d66bff, #ff6bbd, #d66bff, #a66bff, #6bb5ff);
      background-size: 200% 100%;
      animation: roearn-rainbow-flow 6s ease-in-out infinite;
      color: white;
      font-size: 14px;
      font-weight: bold;
      ...
      cursor: pointer;
    `;
    catalogBtn.addEventListener('click', dismiss);

    modal.appendChild(icon);
    modal.appendChild(title);
    modal.appendChild(body);
    modal.appendChild(catalogBtn);
